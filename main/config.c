...
void config_show()
{
    ...
    ESP_LOGI(TAG, "  cfg_version=%u", config->cfg_version);
    ESP_LOGI(TAG, "  lcd_pwr=%u", config->lcd_pwr);
    ESP_LOGI(TAG, "  gps_pwr=%u", config->gps_pwr);
    ESP_LOGI(TAG, "  scd4x_pwr=%u", config->scd4x_pwr);
    ESP_LOGI(TAG, "  wifi_pwr=%u", config->wifi_pwr);
    ESP_LOGI(TAG, "  mode_pwr=%u", config->mode_pwr);
    ESP_LOGI(TAG, "  wifi_auto_connect_idx=%u", config->wifi_auto_connect_idx);
    ESP_LOGI(TAG, "  wifi_auto_connect=%u", config->wifi_auto_connect);
    ESP_LOGI(TAG, "  tcp_auto_start=%u", config->tcp_auto_start);
    ESP_LOGI(TAG, "  ftp_auto_start=%u", config->ftp_auto_start);
    ESP_LOGI(TAG, "  auto_record=%u", config->auto_record);
    ESP_LOGI(TAG, "  scd4x_auto_adjust=%u", config->scd4x_auto_adjust);
    ESP_LOGI(TAG, "  cfg_locked=%u", config->cfg_locked);
    //ESP_LOGI(TAG, "  mqtt_broker=%s", config->mqtt_broker);
    //ESP_LOGI(TAG, "  mqtt_auto_connect=%u", config->mqtt_auto_connect);
}
