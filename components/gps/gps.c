...
//Power save modes
const unsigned char ubxSoftReset[] = { 0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00,
                                       0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0xFF, 0xFF, 0x00, 0x00,
                                      0x07,
                                      0x1F, 0x9E }; // Soft Reset
const uint8_t ubxPartialReset[] = { 0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00,
                                    0x1F, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   0x1F, 0x1F, 0x00, 0x00,
                                   0x07,
                                   0x9F, 0xDE };
const unsigned char ubxFullReset[] = { 0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00,
                                       0xFF, 0xFF, 0xFF, 0xFF,
                                      0x00, 0x00, 0x00, 0x00,
                                      0xFF, 0xFF, 0xFF, 0xFF,
                                      0x07, 0x21, 0x89 }; // Full Reset
const unsigned char ubxPSM[] = { 0xB5, 0x62, 0x06, 0x11, 0x01, 0x00, 0x01, 0x1A, 0x91 }; // Power Save Mode
const unsigned char ubxEM[] =  { 0xB5, 0x62, 0x06, 0x11, 0x02, 0x00, 0x08, 0x01, 0x22, 0x92 }; // Eco Mode
const unsigned char ubxMPM[] = { 0xB5, 0x62, 0x06, 0x86, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x94, 0x00 }; // Max Performance Mode
const unsigned char ubxSleep[] = {0xB5, 0x62, 0x02, 0x41, 0x08, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x4D, 0x3B}; // Sleep mode

static hw_serial_t *gps_serial = NULL;
...
static void gps_sensor_task(void *arg)
{
    ...
    while (true) {
        ...
        if (cnt < rxBytes) {
            //ESP_LOGW(sensor->name, "#OFF=%u %s", cnt, p);
...
void gps_dump_values(gps_sensor_t *sensor, bool force)
{
    ...
    if (force || sensor->debug & 2) {
        ...
        // GSV-GNSS Satallites in View
        ESP_LOGI(TAG, "GSV: %u num_sv=%u", gsv->cnt, gsv->num_sv);
        for (int i = 0; i < 9; i++) {
            gps_gsv_sat_t *sat = &gsv->sats[i];

            if (sat->svid == 0) break;
            ESP_LOGI(TAG, "GSV-SAT%u: svid=%u elv=%u az=%u snr=%u",
